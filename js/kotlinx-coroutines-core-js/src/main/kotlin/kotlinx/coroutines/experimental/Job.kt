...
public actual interface Job : CoroutineContext.Element {
     ...
     */
    public actual fun invokeOnCompletion(onCancelling: Boolean = false, handler: CompletionHandler): DisposableHandle
...
) : RuntimeException("$message: caused by $cause")
...
) : CancellationException(if (cause == null) message else "$message: caused by $cause") {
...
public open class JobSupport(active: Boolean) : Job {
     ...
     * @suppress **This is unstable API and it is subject to change.**
     */
    internal fun updateState(expect: Incomplete, proposedUpdate: Any?, mode: Int) {
        check(state == expect)
...
internal abstract class JobNode<out J : Job>(
    ...
    abstract fun invoke(reason: Throwable?) // CompletionHandler
}
